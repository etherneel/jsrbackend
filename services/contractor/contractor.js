...
import contractorModel from "../../collections/contractor";
...
export const addContractor = async (req, mainUserId, createdBy) => {
  const { email } = req.body;
    ...
    let contractorData = await contractorModel.findOneQuery({ email: email });
    if (contractorData) {
      throw new Error("Email Address Already Exists!");
    } else {
      let project = await contractorModel.saveQuery(req.body);
...
export const getcontractor = async (req, mainUserId, createdBy) => {
  ...
  let totalrecord = await contractorModel.findWithCount(where);
...
export const getcontractorwithid = async (req, mainUserId, createdBy) => {
  ...
  let result = await contractorModel.findOneQuery(where);
  if (result) {
...
export const deletecontractor = async (req, mainUserId, createdBy) => {
  let cid = req.body.id;
    ...
    let contractordata = await contractorModel.findByConditionAndUpdate(where, {
      isDeleted: true,
    });
...
export const updatecontractor = async (req, mainUserId, createdBy) => {
  ...
  if (req.body.email == "" || !req.body.email) {
    ...
    let contractorData = await contractorModel.findOneQuery({
