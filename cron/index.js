...
const levelrewaramountDaily = async (req) => {
    ...
    let data = await rewardamountModel.findQuery(stakingDataWhere);
    ...
    for (let entry of data) {
        ...
        let dailylevvelReward = await levelrewaramountDailyModel.saveQuery(addData);
...
/*************************** rewardAmountDilyAdd code start ***************************/
// const rewardAmountDilyAdd = async (req) => {
//     const planeAmounts = [25, 50, 100, 500, 1000, 5000, 10000];
...
//       let stackData = await rewardamountModel.findQuery(stakingDataWhere);
...
//             rewardamount: dataItem.rewardamount,
//             note: "Daily Rewards " + dataItem.planeAmount
//           };
  
//           let dailyReward = await rewarAmountDailyModel.saveQuery(dataToAdd);
...
const processTeamCounts = async (teamCounts, threshold, amount) => {
    ...
    const findDataTeamAmount = await TeamAmountModel.findQuery({ walletAddress: { $in: walletAddresses } });
    ...
    for (const walletAddress of walletAddresses) {
        ...
        if (!existingData) {
            ...
            let result = await TeamAmountModel.saveQuery(dataToAdd);
